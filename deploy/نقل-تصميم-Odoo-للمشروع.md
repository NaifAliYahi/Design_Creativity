...
## ما الذي أُضيف من Odoo مؤخراً

- **extraCodes** — أكواد/نصوص إضافية + طبقات `code` متعددة
- **تعرف على المناطق الثلاث** — `layout-detect.ts` (نفس خوارزمية `netcard_layout_detect.py`)
- **بحث في القوالب** — في معرض القوالب

## الكatalog والثيمات (مثل Odoo)

- **فلاتر:** نوع / محفظة / مجموعة / بحث — `catalog-defaults.json` + `netcardCatalog` في `store.json`
- **إعدادات القوالب:** تبويب الموظف — رفع دفعة، أنواع، محافظ، إدارة
- **ثيمات:** brand / odoo / teal / navy — `localStorage` (`nc-theme`)

## ما لم يُنقل بعد (اختياري)

- تبويب **استيراد الشبكات** الكامل (Excel + mapping) كما في Odoo
- مزامنة حية مع API Odoo بدون JSON محلي
