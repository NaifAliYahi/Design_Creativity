...
| ✅ يعمل (للعملاء) | ❌ لا يعمل على Netlify |
|-------------------|------------------------|
| `/design` — تصميم وتنزيل بطاقات | `/login` — تسجيل دخول الموظفين |
| اختيار القوالب والتصدير | تعديل مواقع الليبلات (للموظفين فقط) |

**الموظفون:** يستخدمون التطبيق **محلياً** أو **Windows Server** — ليس رابط Netlify.

---

## تصميم القوالب للعملاء (مهم)

1. الموظف يصمّم مواقع الليبلات من **http://localhost:5173/login** → **قالب التصميم** (`/studio`)
2. بعد الانتهاء، نفّذ:
```powershell
cd "D:\windsurf\appCustomer\web"
npm run export:layouts
git add public/netcard/bundled-layouts.json
git commit -m "تحديث مواقع الليبلات للعملاء"
git push
```
3. Netlify يعيد البناء تلقائياً — العملاء يرون المواقع الصحيحة على `/design`
